    ...
    def get(self):
        ...
        for col in self.data.columns:
            fig.add_trace(go.Box(
                y=self.data[col],
                name=method_labels.get(col, col),
                marker_color=method_colors.get(col, None)
            ))

        title_map = {"recall": "Recall Score", "f1": "F1 Score", "roc": "ROC-AUC Score"}
        fig.update_layout(
            template='plotly_white',
            title_text=title_map.get(self.scoring, self.scoring)
        )
        fig.show()
...
class Seaborn_Visualization_result():
    ...
    def get(self):
        ...
        sns.boxplot(data=data, palette=palette[:len(data.columns)])

        title_map = {"recall": "Recall Score", "f1": "F1 Score", "roc": "ROC-AUC Score"}
        plt.title(title_map.get(self.scoring, self.scoring))
        plt.show()
